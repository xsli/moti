...
5. **As soon as the source is stable, background the build gates.** Kick off
   ...
   you finish.
6. **Background the brand-asset pass too (custom-card apps).** If the app gets
   a custom card per the **`og`** skill (games of every kind, whimsical/creative
   apps, brand-forward pages — not plain utilities), launch that pass as a
   background `task` **as soon as the app's name and palette are settled —
   typically right after scaffolding (step 3)**, not at QA time: give the task
   the `og` skill and sole ownership of `public/` brand assets plus
   `src/lib/og/site.json` (see § Parallel work), keep building, then
   `wait_tasks` and integrate its edits before the final verify. The `og`
   skill governs the details; this step is the trigger — image generation is
   pure waiting, and kicking it off after the app is built serializes minutes
   behind a finished app. Do **not** put `og:*` / `twitter:card` in `__root.tsx`
   — the injector owns those tags.
