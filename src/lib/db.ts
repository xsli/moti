...
async function createPgliteSql(): Promise<Sql> {
  ...
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    async function clearStalePid(dir: string): Promise<void> {
      const pidFile = join(dir, "postmaster.pid");
      try {
        const raw = await readFile(pidFile, "utf8");
        const pid = Number(String(raw).trim().split(/\s|\n/)[0]);
        if (!(await pidAlive(pid))) await unlink(pidFile);
      } catch {
    ...
    async function claimOwner(): Promise<"ours" | "busy"> {
      ...
      try {
        ...
        if (await pidAlive(pid)) return "busy";
      } catch {
        /* none */
      }
    ...
    async function open(dir: string) {
      ...
      await clearStalePid(dir);
      const pg = new PGlite(dir, options);
      await pg.waitReady;
      await pg.exec(
        "create table if not exists _migrations (name text primary key, applied_at timestamptz not null default now())",
      );
    ...
    try {
      ...
      console.warn("[db] PGLite open failed, clearing stale lock", first);
      await clearStalePid(dataDir);
      try {
        return await open(dataDir);
      } catch (second) {
        if (owner === "busy") {
          console.error("[db] PGLite in use by another process; not wiping", second);
          throw second;
        }
        console.error("[db] PGLite data dir unreadable, moving aside", second);
        try {
          await rename(dataDir, `${dataDir}.broken-${Date.now()}`);
        } catch {
          /* already moved */
        }
        return await open(dataDir);
...
if (typeof window === "undefined" && dbSource === "pglite" && !isServerlessDeploy()) {
