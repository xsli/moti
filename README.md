...
给 Agent 提供 PDF 路径、目标大组和是否生成答案，例如：

> 请把这份 PDF 按讲次录入“大组名称”，小组标题从讲义提取，例题和习题分别打标签，保留跨页题和配图，并生成答案解析。先试录第一讲，核验后再继续。

具体流程见 [Agent PDF 批量导入说明](docs/agent-pdf-import.md)。项目根目录的 [AGENTS.md](AGENTS.md) 也提供了自动接手入口。

### JSON 备份

本子首页右上角 **导出** / **导入**。导出文件名形如 `解集备份-2026-08-29.json`，UTF-8 JSON。导入内容会写入本机数据库。
